...
def get_estimate(model, noisy, laminput, args):
...
def _estimate_and_save(model, noisy_signals, laminput, filenames, out_dir, args):
    estimate = get_estimate(model, noisy_signals, laminput, args)
    save_wavs(estimate, noisy_signals, filenames, out_dir, sr=model.sample_rate)


def enhance(args, model=None, laminput=0.1, local_out_dir=None):
    # Load model
    if not model:
            ...
            if args.device == 'cpu' and args.num_workers > 1:
                pendings.append(
                    pool.submit(_estimate_and_save,
                                model, noisy_signals, laminput, filenames, out_dir, args))
            else:
                # Forward
                estimate = get_estimate(model, noisy_signals, laminput, args)
                save_wavs(estimate, noisy_signals, filenames, out_dir, sr=model.sample_rate)
