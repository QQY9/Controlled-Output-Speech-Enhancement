...
class BLSTM(nn.Module):
    def __init__(self, dim, layers=2, bi=True):
        super().__init__()
        self.layer1 = nn.LSTM(bidirectional=bi, num_layers=1, hidden_size=dim, input_size=dim)
        self.layer2 = nn.LSTM(bidirectional=bi, num_layers=1, hidden_size=dim, input_size=dim)
        self.linear = None
        self.ftrans = ContitionalInstanceNorm2d()
        self.lamtrans1 = ConditionerNet1()
        self.lamtrans2 = ConditionerNet2()

        if bi:
            self.linear = nn.Linear(2 * dim, dim)

    def forward(self, x, laminput):
        gammas = self.lamtrans1(laminput)
        betas = self.lamtrans2(laminput)
     
        x, hidden = self.layer1(x)
        x = self.ftrans(x,gammas[:,0:768],betas[:,0:768])
        x, hidden = self.layer2(x)
        x = self.ftrans(x,gammas[:,768:1536],betas[:,768:1536])
        if self.linear:
            x = self.linear(x)
        return x, hidden
...
class ConditionerNet1(nn.Module):
    def __init__(self, n_outputs=1536, n_inputs=1, hdim=512):
        "3 input channels- alpha_s1, alpha_s2, alpha_s3"
        "need to output gamma and beta for feature map in T network"

        super(ConditionerNet1, self).__init__()
        self.dense_input = nn.Sequential(nn.Linear(n_inputs, hdim), nn.ReLU())
        self.dense1 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
        self.dense2 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
        self.dense3 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
        self.dense4 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
        self.dense_out = nn.Linear(hdim, n_outputs)
...
class ConditionerNet2(nn.Module):
    def __init__(self, n_outputs=1536, n_inputs=1, hdim=512):
        "3 input channels- alpha_s1, alpha_s2, alpha_s3"
        "need to output gamma and beta for feature map in T network"

        super(ConditionerNet2, self).__init__()
        self.dense_input = nn.Sequential(nn.Linear(n_inputs, hdim), nn.ReLU())
        self.dense1 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
        self.dense2 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
        self.dense3 = nn.Sequential(nn.Linear(hdim, hdim), nn.ReLU())
...
class ContitionalInstanceNorm2d(th.nn.Module):
    def __init__(self):
    ...
    def forward(self, x, weight1, weight2):
        gammas = weight1
        betas = weight2
        #y = x.permute(1,0,2)*gammas + betas # applying reparameterisation trick
        y = x*gammas + betas 
