    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            if not cross_valid:
                ...
                noise, clean = sources
                noisy = noise + clean
            laminput =  (torch.round(8*torch.rand(noisy.size(0), 1))/10+0.1).cuda()
            estimate = self.dmodel(noisy, laminput)
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                if self.args.stft_loss:
                    mag_loss = self.mrstftloss(estimate.squeeze(1), clean.squeeze(1), laminput)
                    loss += mag_loss
