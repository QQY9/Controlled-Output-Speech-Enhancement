...
def run(args):
    ...
    from denoiser.test import Test
    ...
    if args.optim == "adam":
        ...
        os._exit(1)
    load_from = 'D:\code\denoiser-main1\outputs\checkpoint.th'
    package = torch.load(load_from, 'cpu')
    # Construct Solver
    model.load_state_dict(package['model']['state'])
    laminput=args.laminput*torch.ones(1,1).cuda()
  
    test = Test(data, model, laminput, optimizer, args)
    test.train()
